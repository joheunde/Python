...
from tkinter import filedialog # 서브 모듈이기 때문에 별도로 import
...
# 파일 추가 기능
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")), initialdir=r"C:\Users\i\Desktop\PythonWorkspace") # 최초의 사용자가 지정한 경로를 보여줌
...
def merge_image():
    ...
    try:
        ...
        # (원본 width) : (원본 height) = (변경 width) : (번경 height)
        #      100     :      60      =       80     :       ?
        #       x      :       y      =       x'     :       y'
        #     xy' = x'y
        #     y' = x'y / x -> 이 식을 적용
